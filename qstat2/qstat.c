...
raw_display_server( struct qserver *server)
{
    ...
    else if ( server_type == TRIBES_SERVER || server_type == TRIBES2_SERVER)  {
...
{
    static char fmt[24]= "%s" "%s%d" "%s%d" "%s%s";
    struct player *player;
...
deal_with_tribes2_packet( struct qserver *server, char *pkt, int pktlen)
{
    ...
    unsigned int maximum_net_protocol;
...
*/
	...
	pkt+= 4;
	if ( query_version == 5)  {
	    maximum_net_protocol= swap_long_from_little( pkt);
	    sprintf( str, "%u", maximum_net_protocol);
	    add_rule(server,"maximum_net_protocol",str);
	    pkt+= 4;
	}
