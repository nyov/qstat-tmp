...
xml_display_server( struct qserver *server)
{
	...
	if ( server->error != NULL)
	{
		...
		server_type == HERETIC2_SERVER || server_type == SOF_SERVER ||
		server_type == GAMESPY_PROTOCOL_SERVER ||
		server_type == TRIBES2_SERVER || server_type == TRIBES_SERVER ||
		server_type == Q_SERVER || server_type == H2_SERVER)
	{
		fprintf( OF, "\t<server type=\"%s\" address=\"%s\" status=\"%s\">\n",
		...
		if ( server_type == Q_SERVER || server_type == H2_SERVER)
		{
...
send_qwmaster_request_packet( struct qserver *server)
{
    ...
    if ( server->type->id != Q2_MASTER)  {
	int packet_len= server->type->master_len;
	char *packet= server->type->master_packet;
